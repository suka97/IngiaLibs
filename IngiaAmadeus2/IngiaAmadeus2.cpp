...
IngiaAmadeus2 * IngiaAmadeus2::instance = NULL;

boolean IngiaAmadeus2::begin(uint8_t pinModeA, uint8_t pinModeB, uint8_t pinModeC, uint8_t pinModeD, uint8_t pinModeE, uint8_t pinModeF) 
{    
    ...
    for( uint8_t i=0 ; i<15 ; i++ ) {
        print(F("www.ingia.com.ar"), 1, IngiaAmadeus2::LCD_CENTER, true, -1, false);
        print(F("AMADEUS M2"), 0, IngiaAmadeus2::LCD_CENTER, false, -1, true);
        delay(200);
    }
    setRain(false);
    lcdClear();
    ...
    for ( uint8_t i=0 ; i<_cantMenus ; i++ ) {
        if ( (getVal(i) < pgm_read_dword(&_valmin[i])) || (getVal(i) > pgm_read_dword(&_valmax[i])) )
    ...
    if ( mantuvoPresonado ) {
        print(F("Configuracion"), 1, IngiaAmadeus2::LCD_CENTER, true);
        delay(1500);
...
void IngiaAmadeus2::cambiarVarMostrando(char *nombre, long *var, long valmin, long valmax, char *unidad, boolean restart) {
...
boolean IngiaAmadeus2::menuParametros() {
...
void IngiaAmadeus2::EEPROM_WriteValues() {
    for ( int i=0 ; i<_cantMenus ; i++ )
        EEPROM.put ( sizeof(long)*i, pgm_read_dword(&_valores[i]) );
}

void IngiaAmadeus2::EEPROM_WriteValue(long value, int i) {
    EEPROM.put ( sizeof(long)*i, value );
}

void IngiaAmadeus2::EEPROM_Clear() {
    for (unsigned int i = 0 ; i < EEPROM.length() ; i++) {
        EEPROM.write(i, 0);
    }
}

boolean IngiaAmadeus2::login(char *password) { 
...
uint8_t IngiaAmadeus2::_getOpcionesSize(char *str, char index) {
    uint16_t i=0;
...
uint8_t IngiaAmadeus2::_splitOpciones(char* str, char** buffer, uint8_t wordSize, char index) {
