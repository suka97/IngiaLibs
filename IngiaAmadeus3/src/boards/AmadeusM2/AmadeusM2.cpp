...
void AmadeusM2::jogEncoder(uint8_t motorNumber, float defaultSpeed) {
...
bool AmadeusM2_Base::cambiarVar(long *var, long valmin, long valmax, int *flagMaxMin) {
...
void AmadeusM2_Base::cambiarString(const char *title, char *buffer, uint8_t size, char *opc) {
    char opc_buffer[100];
    char *m_opc = opc;
    if ( opc == NULL ) {
        uint8_t i;
...
bool AmadeusM2_Base::askYesNo(const char *title) {
    setIncrementalAcceleration(false);
    lcdClear();
...
bool AmadeusM2_Base::login(char const *password) { 
...
void AmadeusM2_Base::jogEncoder_Base(AccelStepper *stepper, String title, float defaultSpeed) {
