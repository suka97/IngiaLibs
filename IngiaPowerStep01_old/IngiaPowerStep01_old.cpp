#include "IngiaPowerStep01_old.h"


//  ---------------------- PowerStep01.ino -------------------------------------------

status_t IngiaPowerStep01_old::Init(void* pInit)
{
...
void IngiaPowerStep01_old::SetDeviceParamsToGivenValues (powerstep01_init_u_t *initPrm) 
{
...
void IngiaPowerStep01_old::Board_Delay(uint32_t mdelay)
{
    delay(mdelay);
}

void IngiaPowerStep01_old::SetRegisterToPredefinedValues(void)
{
...
void IngiaPowerStep01_old::CmdSetParam(uint8_t param, uint32_t value)
{
  uint32_t loop;
...
void IngiaPowerStep01_old::WriteBytes(uint8_t *pByteToTransmit, uint8_t *pReceivedByte)
{
...
uint8_t IngiaPowerStep01_old::Board_SpiWriteBytes(uint8_t *pByteToTransmit, uint8_t *pReceivedByte)
{
    return (uint8_t) (ReadWrite(pReceivedByte, pByteToTransmit, numberOfDevices) == COMPONENT_OK ? 0 : 1);
}

status_t IngiaPowerStep01_old::ReadWrite(uint8_t* pBufferToRead, uint8_t* pBufferToWrite, uint16_t NumBytes)
{   
...
void IngiaPowerStep01_old::FetchAndClearAllStatus(void)
{
  uint8_t loop;
...
void IngiaPowerStep01_old::SendCommand(uint8_t command, uint32_t value)
{
  uint32_t loop;
  uint8_t maxArgumentNbBytes = 0;
  bool itDisable = FALSE;
...
uint32_t IngiaPowerStep01_old::CmdGetParam(powerstep01_Registers_t param)
{
...
int32_t IngiaPowerStep01_old::ConvertPosition(uint32_t abs_position_reg)
{
...
bool IngiaPowerStep01_old::SetAnalogValue(powerstep01_Registers_t param, float value)
{
...
uint16_t IngiaPowerStep01_old::CmdGetStatus(void)
{
  uint16_t status = 0;
  uint32_t loop;
  uint8_t spiIndex = numberOfDevices - deviceInstance - 1;
...
int IngiaPowerStep01_old::spi_read_write(uint8_t* pBufferToRead, uint8_t* pBufferToWrite, uint8_t ssel, uint16_t NumBytes)
{
    //    /* Check data format */
    //    if(_bits != 8) return -1;
